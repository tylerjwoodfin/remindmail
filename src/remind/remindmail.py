...
class RemindMail:
    ...
    def generate(
        ...
        force: bool = False,
        dry_run: bool = False,
        generate_date: int = None,
        generate_type: GenerateType = GenerateType.EMAIL,
    ):
        """
        ...
        if (
            not (
                today_index != day_of_month_reminders_generated
                and datetime.today().hour > 3
            )
            and not force
            and not dry_run
        ):
        ...
        remindmd_file = (
            RemindMailUtils.cab.get_file_as_array("remind.md", "notes") or []
        )
        ...
        for index, item in enumerate(_remindmd_file):
            ...
            if not "%" in token and not "any" in token:
                try:
                    ...
                    try:
                        parsed_date = parse(f"{token}day", fuzzy_with_tokens=True)
            ...
            if is_match:
                if not is_command:
                    if generate_type == GenerateType.EMAIL and not dry_run:
                        RemindMailUtils().send_email(
                            item.split(" ", 1)[1], item_notes, "remind.md"
                        )
                    if dry_run and generate_type == GenerateType.EMAIL:
                        RemindMail().log_msg(
                            f"Test Mode. Not Sending: {item.split(' ', 1)[1]}",
                            level="debug",
                        )
                ...
                if token_after == 1:
                    if not dry_run:
                        ...
                        RemindMail().log_msg(
                            f"(Test Mode. Not deleting {item})", level="debug"
                        )
                elif token_after > 1:
                ...
                if is_command:
                    if not dry_run:
                        ...
                        RemindMail().log_msg(
                            f"Test Mode. Not executing {command})", level="debug"
                        )

        RemindMailUtils.cab.write_file(
    ...
    def show_date(self, date: str = None):
        ...
        date_to_show = date_to_show.replace(hour=0, minute=0, second=0, microsecond=0)

        RemindMail().generate(
            force=True,
            dry_run=True,
            generate_date=date_to_show,
    ...
    def parse_query(
        self, query=None, manual_message="", manual_date="", noconfirm=False
    ):
        """
        ...
        def strip_to(query):
            ...
            if (
                query.startswith(" to ")
                or query.startswith("to ")
                or query.startswith("day to ")
            ):
        ...
        def parse_date(query: str):
            ...
            if "tomorrow" in query:
                query = (datetime.now() + timedelta(days=int(is_after_4am))).strftime(
                    "%F"
                )
            ...
            if is_after_4am and current_day.lower() in query.lower():
                query_date = (datetime.now() + timedelta(days=7)).strftime("%F")
                query = re.sub(
                    r"\b" + re.escape(current_day) + r"\b",
                    query_date,
                    query,
                    flags=re.IGNORECASE,
                )
        ...
        def report_query():
            ...
            RemindMailUtils().send_email(
                f"Bad Query: {today}",
                "<br>".join(RemindMailUtils.query_trace).replace("\n", "<br>"),
                False,
                is_quiet=False,
            )
        ...
        is_recurring_options = [
            "every [0-9]+",
            "every week",
            "every month",
            "every day",
        ] + [f"every {day}" for day in weekdays]

        is_recurring = any(
            re.findall("|".join(is_recurring_options), query, flags=re.IGNORECASE)
        )
        ...
        # handle "in n months"
        _months = re.findall("in [0-9]+ months|in 1 month", query, flags=re.IGNORECASE)

        if _months:
            _number_of_months = int(re.search(r"\d+", _months[0]).group())
            _newdate = datetime.now().date() + relativedelta(months=_number_of_months)
            ...
            if is_recurring:
                ...
                _frequency = (
                    f"{f'{_number_of_months} ' if _number_of_months > 1 else ''}"
                    f"{'month' if _number_of_months == 1 else 'months'}"
                )
                query_time_formatted = (
                    f"every {_frequency} starting {_newdate.strftime('%B %d')}"
                )
            else:
        ...
        # handle "in n weeks"
        _weeks = re.findall("in [0-9]+ weeks|in 1 week", query, flags=re.IGNORECASE)
        if _weeks:
            ...
            for day in day_prefixes:
                if re.search(day, query, flags=re.IGNORECASE):
                    ...
                    query_time_token = re.sub(
                        "on|next|day", "", day, flags=re.IGNORECASE, count=1
                    ).strip()
        ...
        if parsed_date and (not query_time_token or manual_date):
            ...
            if manual_message:
                ...
                if len(parsed_date[1]) > 1:
                    query = "".join(get_larger(parsed_date[1][0], parsed_date[1][1]))
        ...
        while response not in ["y", "n", "r", "l", "t", "j", "m"]:
            ...
            if response == "p":
                ...
                RemindMailUtils().create_jira_issue(
                    summary=query, description=query_notes or ""
                )
                return
        ...
        if query_time_formatted == "right now" and response == "y":
            RemindMailUtils().send_email(
                query.strip(), query_notes, "RemindMail", is_quiet=True
            )
            return
    ...
    def manual_reminder(self, manual_message="", manual_date="", noconfirm=False):
        ...
        RemindMail().parse_query(
            manual_message=reminder_message,
            manual_date=reminder_date,
            noconfirm=noconfirm,
        )
